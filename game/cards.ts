...
export const CARDS: Record<CardId, Card> = {
  ...
  loiter_drone: variant(
    ...
    '前沿巡飞待机，确认炮位后俯冲。',
    {
      ...
      tag: '无人机 · 巡飞猎炮',
      detail:
        '3费40生命，前沿往返待机60秒。只锁定420内可见载具或炮位，优先火炮，连续确认2秒后实体俯冲。撞击140伤、半径44，对甲×1.25、对步兵×0.7；不追单兵、不伤基地。烟幕可打断确认；俯冲后失去视野只撞最后锁点，可被防空或建筑拦截。警戒可改巡飞位置，撤退停止锁定，俯冲不可撤销；超时消耗，不返还卡牌。',
    },
  ),
  interceptor: variant(
    'helicopter',
  ...
  rapid_insertion: variant(
    ...
    '3 人空降快援，落地后有 8 秒冲刺；首发射击结束加速',
    {
      ...
      tag: '空降 · 高速穿插',
      detail:
        '3费3人150生命，基础移速92。直接伞降指定区域，落地后8秒正常推进速度乘1.8，首次射击结束加速；撤退不享受加速。适合快速填补防线或抢占空地，仍会停下来处理前方已发现敌军。侦察跳降和反甲伞兵没有此冲刺能力。',
    },
  ),
  ...
  recon_jump: variant(
    ...
    '2 人空降侦察，落地警戒、不自行追击，可另行下令',
    {
      ...
      tag: '空降 · 前沿侦察',
      detail:
        '2费2人100生命，观察850、射程700。伞降后原地警戒并共享正常视野，不自行追击；可选中小组另行下令。不提供先导小组的伞降引导，也没有快速穿插的落地冲刺。',
    },
  ),
  // ── v120 炮火支援流派 ─────────────────────────────────────────────
...
Object.assign(CARDS.tank, {
  rate: 4.8,
  tag: '主战 · 优先猎甲',
  description: '主炮优先猎甲，同轴独立压制步兵。',
  detail: '650生命。主炮装填4.8秒，高爆80伤，穿甲390伤。主炮优先选择可见、射程内能命中的装甲，并集中击毁受损车辆；同轴机枪独立压制步兵。与轻坦清除反甲手、重坦压制阵地的分工不同。',
});
Object.assign(CARDS.light_tank, {
  rate: 3.8,
  tag: '轻装甲 · 支援猎杀',
  description: '优先打反甲操作手、轻型车辆，再处理其他目标。',
  detail: '420生命。主炮装填3.8秒，高爆58伤，穿甲300伤。优先打可见且能命中的反甲步兵与反坦克炮，其次轻型车辆、机枪和迫击炮操作手；护卫不被误认成操作手。220内出现敌装甲时先自卫，同轴机枪独立开火。不会为寻找偏好目标穿过未清理的敌军。',
});
Object.assign(CARDS.heavy_tank, {
  rate: 6.4,
  tag: '重装甲 · 阵地压制',
  description: '优先炮位和密集步兵，近身装甲威胁优先自卫。',
  detail: '950生命。主炮装填6.4秒，高爆110伤，穿甲510伤。优先打可见且能命中的炮位，其次32距离内聚集至少3人的步兵，再处理远处装甲或零散步兵；220内敌装甲优先自卫。同轴机枪独立开火，不获得穿墙或额外伤害加成。',
});
...
Object.assign(CARDS.mortar, {
  rate: 7,
  detail: '4费3人150生命，每7秒全组66伤害，射程180–820。在同一阵位至少打出两轮、自身位置被敌方声测锁定后才搬迁；同班其他炮手的首轮炮声不会让自己提前收炮。驻守时持续射击，近敌进入死角时后撤。',
});
Object.assign(CARDS.mortar_carrier, {
  rate: 8,
  description: '打一发就换位，驻守可取消',
  detail: '4费270生命，每8秒42伤，射程170–780。每次开炮后利用装填间隙向己方倒车换位最多96距离，车头仍朝敌；停稳0.55秒后才能再射。保留80射程余量，可退空间不足48时留在原地。驻守命令取消换位，近敌进入死角时仍按原规则后撤。徒步迫击炮组则在被声测定位后才转移。',
});
Object.assign(CARDS.artillery, {
  rate: 12,
